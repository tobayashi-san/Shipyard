...
`;
  ...
  // The OpenTofu state owns the virtual machine lifecycle, while Fleet host
  ...
  function finalizeFleetProxmoxVmDestroy({ workspace, vm, logMeta = {} }) {
    ...
    syncFleetWorkspace(workspace, `Destroy Fleet Proxmox VM ${vm.name}`);
  }
        ...
        const finish = async () => {
            ...
            try {
              ...
              if (sync.source === 'outputs' && !sync.authoritative && sync.servers.length === 0) {
                ...
                if (postDeploy.started) {
                  emitMeta(`[Fleet] Post-deploy complete: ${postDeploy.succeeded} succeeded, ${postDeploy.failed} failed.`);
                }
              }
            } catch (err) {
          ...
          if (success && action === 'destroy') {
            try {
              ...
              emitMeta(`[Shipyard] Detached ${result.detached} Fleet host(s) from this deployment; the inventory entries were kept.`);
            } catch (err) {
          ...
          if (success && action === 'destroy_vm') {
            try {
              ...
              emitMeta(`[Shipyard] VM ${vmToDestroy.name} destroyed and removed from this deployment. Fleet inventory entries were kept.`);
            } catch (err) {
              log.error({ err, workspace: workspace.name, vm: vmToDestroy.name }, 'OpenTofu VM destroy finalization failed');
              emitMeta(`[Shipyard] VM destroy finished, but Fleet cleanup failed: ${err.message}`);
            }
          }
