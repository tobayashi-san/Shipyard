...
`;
  ...
  async function loadProxmoxInfrastructureSummary(environmentId) {
    removeOrphanedServerMappings(db);
    const { grouped, warnings } = collectProxmoxInfrastructureGroups(environmentId);
    const settled = await Promise.allSettled([...grouped.values()].map(async group => {
  ...
  const infrastructureSummaryRefreshes = new Map();
  const infrastructureSummaryRefreshAttempts = new Map();
  const INFRASTRUCTURE_SUMMARY_MAX_AGE_MS = Math.max(
    5_000,
    Number.parseInt(process.env.PROXMOX_SUMMARY_MAX_AGE_MS || '30000', 10) || 30_000
  );

  function infrastructureSummaryKey(environmentId) {
    return `tofu.infrastructure.summary.${encodeURIComponent(environmentId)}`;
  }

  function infrastructureSummarySourceVersion(environmentId) {
  ...
  function summarizeInfrastructure(infrastructure, environmentId) {
    return {
      environment_id: environmentId,
      updated_at: new Date().toISOString(),
      source_version: infrastructureSummarySourceVersion(environmentId),
  ...
  function readInfrastructureSummary(environmentId) {
    try {
      const parsed = JSON.parse(db.settings.get(infrastructureSummaryKey(environmentId)) || 'null');
  ...
  function refreshInfrastructureSummary(environmentId) {
    const running = infrastructureSummaryRefreshes.get(environmentId);
    if (running) return running;
    infrastructureSummaryRefreshAttempts.set(environmentId, Date.now());
    const refresh = loadProxmoxInfrastructureSummary(environmentId)
      .then(infrastructure => {
        ...
        if ((infrastructure.warnings || []).length > 0) {
          if (!readInfrastructureSummary(environmentId)) {
            throw new Error(infrastructure.warnings[0] || 'Proxmox inventory could not be loaded.');
          }
          return readInfrastructureSummary(environmentId);
        }
        const summary = summarizeInfrastructure(infrastructure, environmentId);
        db.settings.set(infrastructureSummaryKey(environmentId), JSON.stringify(summary));
        return summary;
      })
      .finally(() => infrastructureSummaryRefreshes.delete(environmentId));
    infrastructureSummaryRefreshes.set(environmentId, refresh);
    return refresh;
  }

  async function getInfrastructureSummary(environmentId) {
    const cached = readInfrastructureSummary(environmentId);
    if (!cached) {
      const summary = await refreshInfrastructureSummary(environmentId);
      return { ...summary, cached: false, refreshing: false };
    }
    const age = Date.now() - Date.parse(cached.updated_at);
    const sourceChanged = cached.source_version !== infrastructureSummarySourceVersion(environmentId);
    const stale = sourceChanged || !Number.isFinite(age) || age >= INFRASTRUCTURE_SUMMARY_MAX_AGE_MS;
    const lastAttempt = infrastructureSummaryRefreshAttempts.get(environmentId) || 0;
    if (stale && (sourceChanged || Date.now() - lastAttempt >= INFRASTRUCTURE_SUMMARY_MAX_AGE_MS)) {
      void refreshInfrastructureSummary(environmentId).catch(error => {
        log.warn({ err: error, environmentId }, 'Could not refresh cached Proxmox infrastructure summary');
      });
    }
    return { ...cached, cached: true, refreshing: infrastructureSummaryRefreshes.has(environmentId) };
  }
