...
function normalizeWindow(body = {}) {
  ...
  const timezone = String(body.timezone || 'Europe/Zurich').trim().slice(0, 80) || 'Europe/Zurich';
  ...
  return {
    ...
    affected_resources: String(body.affected_resources || '').trim().slice(0, 1000),
    timezone,
    owner: String(body.owner || '').trim().slice(0, 120),
  };
}
...
router.post('/', guard('canEditMaintenance'), (req, res) => {
  try {
    ...
    const id = db.uuidv4();
    db.db.prepare('INSERT INTO maintenance_windows (id, environment_id, name, starts_at, ends_at, description, affected_resources, timezone, owner, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
      .run(id, environmentId, value.name, value.starts_at, value.ends_at, value.description, value.affected_resources, value.timezone, value.owner, req.user?.username || '');
...
router.put('/:id', guard('canEditMaintenance'), (req, res) => {
  ...
  try {
    const value = normalizeWindow(req.body);
    db.db.prepare('UPDATE maintenance_windows SET name = ?, starts_at = ?, ends_at = ?, description = ?, affected_resources = ?, timezone = ?, owner = ? WHERE id = ?')
      .run(value.name, value.starts_at, value.ends_at, value.description, value.affected_resources, value.timezone, value.owner, existing.id);
