...
export function ImportProxmoxVmDialog({ connectionId, environmentId, vm, open, onOpenChange }: { connectionId: string; environmentId: string; vm: { name: string; node_name: string; vm_id: number }; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const importMutation = useMutation({
    mutationFn: async () => {
      ...
      if (temporaryPassword) await apiFetch('/system/deploy', { method: 'POST', body: { ip_address: result.server.ip_address, ssh_user: sshUser, ssh_port: Number(sshPort), password: temporaryPassword } });
      return result;
    },
    onSuccess: () => { showToast(temporaryPassword ? 'VM wurde übernommen und der Fleet-SSH-Key installiert.' : 'VM wurde als Fleet-Host übernommen.', 'success'); void queryClient.invalidateQueries({ queryKey: ['servers'] }); void queryClient.invalidateQueries({ queryKey: ['opentofu', 'infrastructure', environmentId] }); onOpenChange(false); },
