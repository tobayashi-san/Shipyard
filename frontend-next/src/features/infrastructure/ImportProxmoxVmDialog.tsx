...
export function ImportProxmoxVmDialog({
  connectionId,
  environmentId,
  vm,
  open,
  onOpenChange,
}: ImportProxmoxVmDialogProps) {
  ...
  const groupsQuery = useQuery({
    ...
    queryFn: () => apiFetch<Folder[]>(`/servers/groups?environment_id=${encodeURIComponent(environmentId)}`),
    enabled: open,
    staleTime: 30_000,
  });
  ...
  const guestIpQuery = useQuery({
    queryKey: ['opentofu', 'proxmox-guest-ip', connectionId, vm.node_name, vm.vm_id],
    queryFn: () => apiFetch<GuestIp>(`/opentofu/proxmox-connections/${encodeURIComponent(connectionId)}/guest-ip?node=${encodeURIComponent(vm.node_name)}&vm_id=${vm.vm_id}`),
  ...
  useEffect(() => {
    if (guestIpQuery.data?.ip_address && !ipAddress) setIpAddress(guestIpQuery.data.ip_address);
  }, [guestIpQuery.data?.ip_address, ipAddress]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] max-w-xl overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            The existing Proxmox {kind} is linked as a host. No VM or container settings are changed and no resources are created.
          </DialogDescription>
        </DialogHeader>
        <form className="space-y-4" onSubmit={event => { event.preventDefault(); importMutation.mutate(); }}>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="IP address">
              <div className="flex gap-2">
                <Input required value={ipAddress} onChange={event => setIpAddress(event.target.value)} placeholder="10.20.1.10" />
                <Button type="button" size="icon" variant="outline" onClick={() => void guestIpQuery.refetch()} disabled={guestIpQuery.isFetching} aria-label="Read virtual machine IP">
            ...
            <Field label="SSH user">
              <Input required value={sshUser} onChange={event => setSshUser(event.target.value)} placeholder="ubuntu" />
            </Field>
            <Field label="SSH port">
              <Input required value={sshPort} onChange={event => setSshPort(event.target.value)} inputMode="numeric" />
            </Field>
          </div>
          <Field label="Folder">
            <select value={groupId} onChange={event => setGroupId(event.target.value)} className="h-9 w-full rounded-sm border bg-background px-2.5 text-[13px]">
          ...
          <div className="rounded-md border border-primary/20 bg-primary/5 p-3">
            ...
            <Label className="mt-3 block" htmlFor="import-vm-password">Temporary password <span className="font-normal text-muted-foreground">(optional)</span></Label>
            <Input id="import-vm-password" className="mt-1.5" value={temporaryPassword} onChange={event => setTemporaryPassword(event.target.value)} type="password" autoComplete="new-password" />
          </div>
          <DialogFooter>
