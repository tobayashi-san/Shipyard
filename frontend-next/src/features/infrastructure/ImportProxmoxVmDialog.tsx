...
export function ImportProxmoxVmDialog({ connectionId, environmentId, vm, open, onOpenChange }: { connectionId: string; environmentId: string; vm: { name: string; node_name: string; vm_id: number; guest_type?: "qemu" | "lxc" }; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const importMutation = useMutation({
    ...
    onSuccess: result => { showToast(result.keyDeployError ? `${kind} adopted. SSH key: ${result.keyDeployError}` : temporaryPassword ? `${kind} adopted and Fleet SSH key installed.` : `${kind} adopted as a managed host.`, result.keyDeployError ? 'warning' : 'success'); void queryClient.invalidateQueries({ queryKey: ['servers'] }); void queryClient.invalidateQueries({ queryKey: ['dashboard'] }); void queryClient.invalidateQueries({ queryKey: ['opentofu', 'infrastructure', environmentId] }); void queryClient.invalidateQueries({ queryKey: ['proxmox-vm-context', connectionId, vm.node_name, vm.vm_id] }); onOpenChange(false); },
