...
export function ImportProxmoxVmDialog({ connectionId, environmentId, vm, open, onOpenChange }: { connectionId: string; environmentId: string; vm: { name: string; node_name: string; vm_id: number; guest_type?: "qemu" | "lxc" }; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const importMutation = useMutation({
    ...
    onSuccess: result => { showToast(result.keyDeployError ? `Guest adopted. SSH key: ${result.keyDeployError}` : temporaryPassword ? 'Guest adopted and Fleet SSH key installed.' : 'Guest adopted as a managed host.', result.keyDeployError ? 'warning' : 'success'); void queryClient.invalidateQueries({ queryKey: ['servers'] }); void queryClient.invalidateQueries({ queryKey: ['dashboard'] }); void queryClient.invalidateQueries({ queryKey: ['opentofu', 'infrastructure', environmentId] }); void queryClient.invalidateQueries({ queryKey: ['proxmox-vm-context', connectionId, vm.node_name, vm.vm_id] }); onOpenChange(false); },
    onError: (error: Error) => showToast(error.message, 'error'),
  });
  const kind = vm.guest_type === 'lxc' ? 'CT' : 'VM';
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[calc(100dvh-2rem)] max-w-xl overflow-y-auto"><DialogHeader><DialogTitle className="flex items-center gap-2"><ServerCog className="h-5 w-5" />Adopt {kind} as managed host</DialogTitle><DialogDescription>The existing Proxmox {kind} is linked as a managed host. No guest settings are changed and no resources are created.</DialogDescription></DialogHeader><form className="space-y-4" onSubmit={event => { event.preventDefault(); importMutation.mutate(); }}><div className="grid gap-3 rounded-md border bg-muted/20 p-3 text-sm sm:grid-cols-3"><div><span className="text-xs text-muted-foreground">{kind}</span><div className="font-medium">{vm.name}</div></div><div><span className="text-xs text-muted-foreground">Node</span><div className="font-mono">{vm.node_name}</div></div><div><span className="text-xs text-muted-foreground">{kind} ID</span><div className="font-mono">{vm.vm_id}</div></div></div><div className="grid gap-4 sm:grid-cols-2"><Field label="Managed host name"><Input required value={name} onChange={event => setName(event.target.value)} /></Field><Field label="IP address"><div className="flex gap-2"><Input required value={ipAddress} onChange={event => setIpAddress(event.target.value)} placeholder="10.20.1.10" /><Button type="button" size="icon" variant="outline" onClick={() => void guestIpQuery.refetch()} disabled={guestIpQuery.isFetching} aria-label="Read guest IP"><RefreshCw className={guestIpQuery.isFetching ? 'animate-spin' : undefined} /></Button></div><p className="mt-1 text-xs text-muted-foreground">{vm.guest_type === 'lxc' ? 'Read from the LXC interface list or enter manually.' : 'Read from the QEMU Guest Agent or enter manually.'}</p></Field><Field label="SSH user"><Input required value={sshUser} onChange={event => setSshUser(event.target.value)} placeholder="ubuntu" /></Field><Field label="SSH port"><Input required value={sshPort} onChange={event => setSshPort(event.target.value)} inputMode="numeric" /></Field></div><Field label="Folder"><select value={groupId} onChange={event => setGroupId(event.target.value)} className="h-8 w-full rounded-sm border bg-background px-2.5 text-[13px]"><option value="">No folder</option>{groups.map(group => <option key={group.id} value={group.id}>{group.name}</option>)}</select></Field><div className="rounded-md border border-primary/20 bg-primary/5 p-3"><div className="flex items-center gap-2 text-sm font-medium"><KeyRound className="h-4 w-4" />Fleet SSH key</div><p className="mt-1 text-xs text-muted-foreground">Fleet installs its configured SSH key using a temporary password. The password is used only for this action and is not stored.</p><Label className="mt-3 block" htmlFor="import-vm-password">Temporary password <span className="font-normal text-muted-foreground">(optional)</span></Label><Input id="import-vm-password" className="mt-1.5" value={temporaryPassword} onChange={event => setTemporaryPassword(event.target.value)} type="password" autoComplete="new-password" /></div><DialogFooter><Button type="button" variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button><Button type="submit" disabled={importMutation.isPending}>{importMutation.isPending ? <RefreshCw className="animate-spin" /> : <ServerCog />}Adopt as managed host</Button></DialogFooter></form></DialogContent></Dialog>;
}
